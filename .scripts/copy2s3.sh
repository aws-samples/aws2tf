...
cmd=$(printf "aws s3 cp /tmp/aws2tf/generated/tf-%s-%s s3://aws2tf-%s-%s/ --recursive" $acc $reg $acc $reg)
echo $cmd
aws s3 cp /tmp/aws2tf/generated/tf-${acc}-${reg} s3://aws2tf-${acc}-${reg}/ --recursive

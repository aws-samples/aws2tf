#!/bin/bash
ttft="aws_vpclattice_service_network"
pref="items"
idfilt="id"

cm="$AWS vpc-lattice list-service-networks"
if [[ "$1" != "" ]]; then
    if [[ "$1" == "sn-"* ]]; then
        cm=$(printf "$AWS vpc-lattice list-service-networks  | jq '.${pref}[] | select(.${idfilt}==\"%s\")' | jq ." $1)
    else
        cm=$(printf "$AWS vpc-lattice list-service-networks  | jq '.${pref}[] | select(.name==\"%s\")' | jq ." $1)
    fi
fi

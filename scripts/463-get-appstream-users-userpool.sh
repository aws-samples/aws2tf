#!/bin/bash
ttft="aws_appstream_user"
pref="Users"
idfilt="UserName"

cm="$AWS appstream describe-users --authentication-type USERPOOL"
if [[ "$1" != "" ]]; then
    cm=`printf "$AWS appstream describe-users --authentication-type USERPOOL | jq '.${pref}[] | select(.${idfilt}==\"%s\")' | jq ." $1`
fi
...
for i in `seq 0 $count`; do
    ...
    terraform import $ttft.${rname} "${cname}/USERPOOL" | grep Importing
    ...
    do
        ...
        if [[ ${t1} == *"="* ]];then
            ...
            if [[ ${tt1} == "id" ]];then 
                skip=1
                printf "lifecycle {\n" >> $fn
                printf "   ignore_changes = [send_email_notification]\n" >> $fn
                printf "}\n" >> $fn
            fi  

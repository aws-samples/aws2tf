#!/bin/bash
ttft="aws_codestarnotifications_notification_rule"
pref="NotificationRules"
idfilt="Arn"

cm="$AWS codestar-notifications list-notification-rules"
if [[ "$1" != "" ]]; then
    cm=`printf "$AWS codestar-notifications list-notification-rules | jq '.${pref}[] | select(.${idfilt}==\"%s\")' | jq ." $1`
fi

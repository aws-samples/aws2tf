#!/bin/bash
if [ "$1" != "" ]; then
    cmd[0]="$AWS dms describe-replication-tasks --filters \"Name=endpoint-id,Values=$1\""
else
    cmd[0]="$AWS dms describe-replication-tasks"

fi

pref[0]="ReplicationTasks"
tft[0]="aws_dms_replication_task"
idfilt[0]="ReplicationTaskIdentifier"
        ...
        for i in `seq 0 $count`; do
            ...
            do
                ...
                if [[ ${t1} == *"="* ]];then
                    ...
                    if [[ ${tt1} == "endpoint_arn" ]];then skip=1;fi
                    if [[ ${tt1} == "replication_task_arn" ]];then skip=1;fi

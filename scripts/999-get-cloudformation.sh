...
for c in $(seq 0 0); do
    ...
    if [ "$count" -gt "0" ]; then
        ...
        for i in $(seq 0 $count); do
            ...
            ($AWS cloudformation get-template --stack-name $cname >data/cft__$rname.json) 2>/dev/null
            if [[ $? -ne 0 ]]; then
            ...
            while IFS= read line; do
                ...
                if [[ ${t1} == *"="* ]]; then
                    ...
                    if [[ ${tt1} == "id" ]]; then
                        t1=$(printf "template_url = file(\"data/cft__%s.json\")" $rname)
                        printf "lifecycle {\n" >>$fn
                        printf "   ignore_changes = [template_url]\n" >>$fn
                        printf "}\n" >>$fn
                    fi
                    ...
                    if [[ ${tt1} == "template_body" ]]; then
                        ...
                        if [[ $tt2 == *"<<-EOT"* ]]; then
                            ...
                            while [[ $breq -eq 0 ]]; do
                                ...
                                if [[ ${t1} == "EOT" ]]; then rbc=$(expr $rbc + 1); fi
                                if [[ $rbc -eq $lbc ]]; then
                                    breq=1
                                    echo "breaking tb"
                                else

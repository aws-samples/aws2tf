#!/bin/bash
if [ "$1" != "" ]; then
    cmd[0]="$AWS appmesh list-virtual-nodes --mesh-name $1" 
else
...
pref[0]="virtualNodes"
tft[0]="aws_appmesh_virtual_node"
idfilt[0]="virtualNodeName"
            ...
            do
                ...
                if [[ ${t1} == *"="* ]];then
                    ...
                    if [[ ${tt1} == "virtual_service_name" ]]; then
                        tt2=`echo $tt2 | tr -d '"'`
                        t1=`printf "%s = aws_appmesh_virtual_service.%s.id" $tt1 $tt2`
                    fi

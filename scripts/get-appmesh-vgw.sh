#!/bin/bash
if [ "$1" != "" ]; then
    cmd[0]="$AWS appmesh list-virtual-gateways --mesh-name $1" 
else
    echo "Mesh name must be set"
    exit
fi

pref[0]="virtualGateways"
tft[0]="aws_appmesh_virtual_gateway"
idfilt[0]="virtualGatewayName"
...
for c in `seq 0 0`; do
    ...
    if [ "$count" -gt "0" ]; then
        ...
        for i in `seq 0 $count`; do
            ...
            ../../scripts/get-appmesh-vgw-routes.sh $1 $cname
        done

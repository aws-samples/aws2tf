#!/bin/bash
if [ "$1" != "" ]; then
    cmd[0]=$(printf "$AWS sagemaker list-app-image-configs | jq '. | select(.AppImageConfigs[].AppImageConfigName==\"%s\")'" $1)
else
    cmd[0]="$AWS sagemaker list-app-image-configs"
fi

pref[0]="AppImageConfigs"
tft[0]="aws_sagemaker_app_image_config"
idfilt[0]="AppImageConfigName"

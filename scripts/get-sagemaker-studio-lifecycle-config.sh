#!/bin/bash
ttft="aws_sagemaker_studio_lifecycle_config"
pref="StudioLifecycleConfigs"
idfilt="StudioLifecycleConfigName"

cm="$AWS sagemaker list-studio-lifecycle-configs"
if [[ "$1" != "" ]]; then
    cm=`printf "$AWS sagemaker list-studio-lifecycle-configs | jq '.${pref}[] | select(.${idfilt}==\"%s\")' | jq ." $1`
fi
